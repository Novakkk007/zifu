...
import {
  ...
  ShenshaTable,
  TenGodsTable,
} from "@/components/bazi-v2/ChartDetails";
...
export default function Bazi() {
  ...
  const initialChart =
    restoredChart?.pillars && restoredChart.input ? restoredChart : null;
    ...
    onSuccess: async data => {
      ...
      if (res.chart) {
        const item = {
          ...
          type: "bazi",
          title:
            chartTitle || `八字排盘 ${new Date().toLocaleDateString("zh-CN")}`,
  ...
  const handleFavorite = () => {
    ...
    const deduped = existing.filter(
      (f: { payload?: unknown }) =>
        JSON.stringify((f.payload as { pillars?: unknown })?.pillars) !==
        signature
    );
  ...
  return (
    <div>
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(to bottom, rgb(var(--silk)), rgb(var(--deep-2)))",
        }}
      />
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(to bottom, rgb(var(--deep-2)), rgb(var(--deep-3)))",
        }}
      />
    </div>
  );
