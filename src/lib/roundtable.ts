...
export function parseRoundTable(text: string): RoundTableResult {
  // 先净化再识别标题：保留换行、回车和制表符，避免异常字符破坏分段。
  text = Array.from(text).filter((char) => {
    const code = char.codePointAt(0)!
    return char === '\n' || char === '\r' || char === '\t' || (
      code >= 0x20 && !(code >= 0x7f && code <= 0x9f) && code !== 0xfffd
    )
  }).join('')
  const seats: RoundTableResult['seats'] = []
  const blocks = text.split(new RegExp(`(?=${SEAT_MARK}|【共识与分歧】|【先生收束】)`))
  ...
  // 先生开场（三句好话）
  let opening = ''
  const om = text.match(new RegExp(`【先生开场】\\s*([\\s\\S]*?)(?=${SEAT_MARK}|【共识与分歧】|【先生收束】|$)`))
