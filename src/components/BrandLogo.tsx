/**
 * 紫府 BrandLogo — 全站唯一 Logo 渲染入口。
 * 图形母版：public/assets/logo-mark.svg（金色线性双球葫芦轮廓 + 内部连续 S 气脉曲线），
 * 字标：「紫府」Noto Serif CJK SC Bold 轮廓（系统 serif 描边转曲，非 AI 字形）。
 *
 * variant: mark（单标） | wordmark（纯字标） | horizontal（横排） | stacked（竖排） | reverse（深底反白横排）
 * theme:   适配底色 indigo（深靛蓝） | black（黑） | ivory（米白浅底） | transparent（透明/未知底）
 * 颜色一律走 CSS token（--zifu-gold-*），组件内不写死任何金色色值。
 */
import type { ComponentPropsWithoutRef } from 'react'

export type BrandLogoVariant = 'mark' | 'wordmark' | 'horizontal' | 'stacked' | 'reverse'
export type BrandLogoTheme = 'indigo' | 'black' | 'ivory' | 'transparent'

export interface BrandLogoProps extends Omit<ComponentPropsWithoutRef<'span'>, 'color'> {
...
/* ---- 母版矢量（与 public/assets/logo-mark.svg 同源） ---- */
export const BRAND_MARK_VIEWBOX = '94 14 324 470'
...
/* ---- 「紫府」字标（Noto Serif CJK SC Bold 转曲，em 框 2240×1000） ---- */
export const BRAND_WORDMARK_VIEWBOX = '0 0 2240 1000'
...
/** 底色适配：金色一律取 token；浅底（米白）降到 700 保对比度 */
const THEME_COLOR: Record<BrandLogoTheme, string> = {
  indigo: 'rgb(var(--zifu-gold-500))',
  black: 'rgb(var(--zifu-gold-500))',
  ivory: 'rgb(var(--zifu-gold-700))',
  transparent: 'rgb(var(--zifu-gold-500))',
}

/** 葫芦母标路径组：stroke=currentColor，可嵌入任意 <svg> 上下文（星盘中心等） */
export function BrandMarkPaths({ strokeWidth = 15 }: { strokeWidth?: number }) {
  return (
    <g
      fill="none"
      stroke="currentColor"
...
export default function BrandLogo({
  ...
  const color = variant === 'reverse' ? '#FFFFFF' : THEME_COLOR[theme]
  const layout: Exclude<BrandLogoVariant, 'reverse'> =
    variant === 'reverse' ? 'horizontal' : variant
