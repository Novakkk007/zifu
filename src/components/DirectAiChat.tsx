  ...
  return (
    <div className="space-y-3">
      {sections.map((section, index) => (
        <p
          key={index}
          className="whitespace-pre-wrap font-serif text-[14.5px] leading-[2]"
        >
          {section}
        </p>
      ))}
...
export default function DirectAiChat({
  ...
  useEffect(() => {
    if (!autoStart || autoStarted.current || !hasAccess || !chartSummary)
      return;
  ...
  const sendQuestion = async (event: FormEvent) => {
    ...
    const nextHistory: DirectChatHistoryMessage[] = [
      ...previousMessages,
      { role: "user", content },
    ];
  ...
  return (
    <div className="rounded-xl border border-gold/40 bg-deep p-5 text-center sm:p-8">
      ...
      {error && (
        <p
          role="alert"
          className="mt-4 text-[12.5px] leading-relaxed text-red-400"
        >
          {error}
        </p>
      )}
        ...
        <div className="mx-auto mt-7 max-w-[760px] border-t border-golddim/25 pt-6 text-left">
                ...
                >
                  ...
                  >
                    <div className="mb-2 flex items-center justify-between gap-3">
                      <p className="text-[11px] tracking-[0.15em] text-golddim">
                        {isVisitor ? "访客" : "先生"}
                      </p>
                      {!isVisitor && (
                        <button
          ...
          >
            <label className="min-w-0 flex-1">
              <span className="mb-2 block text-[12px] text-silkmuted">
                接着问先生
              </span>
              <textarea
                value={question}
