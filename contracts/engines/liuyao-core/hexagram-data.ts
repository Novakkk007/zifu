/**
 * 六十四卦基础数据（本卦/变卦/互卦查找 + 卦辞爻辞）。
 * 由 src/data/hexagrams.json 生成的类型化副本；原 JSON 保持不动，供其他消费方使用。
 * 卦辞爻辞皆出《周易》公版原文。lines 自下而上，1=阳 0=阴。
 */

export type TrigramName = '乾' | '兑' | '离' | '震' | '巽' | '坎' | '艮' | '坤'

export interface HexagramData {
  /** 序卦次（1-64，京房无关，仅《周易》卦序） */
  id: number
  name: string
  upper: TrigramName
  lower: TrigramName
  /** 六爻阴阳，自下而上（初爻在 index 0），1=阳 0=阴 */
  lines: number[]
  /** 卦辞（《周易》原文） */
  gua: string
  /** 六爻爻辞（《周易》原文，自下而上） */
...
export const HEXAGRAMS: HexagramData[] = [
  ...
  },
  {
    ...
    "gua": "坤：元亨，利牝马之贞。君子有攸往，先迷后得主，利。西南得朋，东北丧朋。安贞吉。",
    "yao": [
      "初六：履霜，坚冰至。",
      "六二：直方大，不习无不利。",
      "六三：含章可贞，或从王事，无成有终。",
  ...
  },
  {
    ...
    "gua": "复：亨。出入无疾，朋来无咎。反复其道，七日来复，利有攸往。",
    "yao": [
      "初九：不远复，无祗悔，元吉。",
      "六二：休复，吉。",
      "六三：频复，厉无咎。",
...
/** lines 键（如 '111111'）→ 卦 */
const BY_KEY = new Map<string, HexagramData>(HEXAGRAMS.map((h) => [h.lines.join(''), h]))

/** 按六爻阴阳（自下而上，1=阳 0=阴）查卦；找不到时抛错 */
export function findHexagramByLines(lines: number[]): HexagramData {
  const hit = BY_KEY.get(lines.join(''))
  if (!hit) throw new Error(`hexagram not found: ${lines.join('')}`)
  return hit
}
